...
public class PersonalInfoTests {
	...
	public void navigatToHomePage() {
		...
		email = "gurtaner_79@yahoo.com";//data.internet().emailAddress();
		...
		github = "https://github.com/TanerGur19";
	...
	public void submitFullApplication() {
		...
		*/
		...
		assertEquals(ip.substring(ip.length()-13), "69.124.51.106");
		
		String appId = driver.findElement(By.xpath("//div[contains (text(), 'Application ID')]")).getText();
		
		int g = driver.
				findElement(By.xpath("//div[contains (text(), 'Gender')]")).getText().contains("Male") ? 1 : 2;
		...
		driver.findElement(By.xpath("//input[@id='login-username']")).sendKeys("gurtaner_79@yahoo.com");
		driver.findElement(By.xpath("//input[@id='login-signin']")).click();
		driver.findElement(By.xpath("//input[@id='login-passwd']")).sendKeys("Bitanem7");
	...
	public void setCertifications(List<String> cert) { // homework  if you create methods because you can use it next time
		...
		for (int i = 0; i < cert.size(); i++) {
			...
			if (num == 1) {
				driver.findElement(By.xpath("//input[@id='Checkbox_" + (i+1) + "']")).click(); // otherwise you need to type 3 times
				// driver.find...
			}			
		}
	}
